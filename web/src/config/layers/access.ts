...
export const ACCESS_LAYERS: readonly LayerDef[] = [
  // ───── Access module (new pillar) ─────────────────────────────────────────
  {
    ...
    freshness: "weekly",
    // FWP unpublishes the Block Management Area FeatureServices outside the
    // hunting season (they 404 until republished), and while unpublished this
    // layer carried an `unavailable` block so it was never added to the map —
    // no failed request, so ArcGIS IdentityManager never popped a sign-in
    // dialog — and the panel showed an honest "offline" state.
    //
    // Verified republished 2026-09-14: the service resolves as "Block
    // Management Area (BMA) Boundaries - 2026 Hunting Season" with 1,587
    // features, so the block is removed and the layer is live again. Restore
    // it (badge "Offline" + a note) if FWP unpublishes at the end of season.
    geometry: "polygon",
