...
- [docs/SWIFTKOTLIN_NATIVE_MIGRATION_GUIDE.md](docs/SWIFTKOTLIN_NATIVE_MIGRATION_GUIDE.md) and [docs/FLUTTER_MIGRATION_GUIDE.md](docs/FLUTTER_MIGRATION_GUIDE.md) — **forward-looking**: what a native rebuild would take, as two native apps or as one Flutter codebase
...
## If the next step is a native rebuild

This repository is a working application, not a prototype to be thrown away —
but "should this be native?" is a fair question to ask of anything shipped in a
web view, and it deserves a real answer rather than a guess. Two companion
documents give one:

- **[docs/SWIFTKOTLIN_NATIVE_MIGRATION_GUIDE.md](docs/SWIFTKOTLIN_NATIVE_MIGRATION_GUIDE.md)** — the migration as two native applications, iOS in Swift and Android in Kotlin.
- **[docs/FLUTTER_MIGRATION_GUIDE.md](docs/FLUTTER_MIGRATION_GUIDE.md)** — the same migration as a single Flutter codebase compiled to both platforms.

They share a structure so they can be read side by side, and both are candid
about what ports cleanly (the API contract, the GIS registry, the layer and
feature-card specifications), what has to be written again (the ~1,440 tests and
the 12 audit gates), and what a rebuild does *not* fix on its own (push
credentials, MyFWP sign-in, upstream GIS stability).

Both also arrive at the same recommendation, which is worth acting on whatever
is decided about the client: **publish a single FWP-owned, pre-styled map
service.** This application configures all 47 of its map layers in the client
because it consumes 27 independent public services and had no other option. One
shared map item would make every current and future version of this app
smaller — and it is the prerequisite for offline *feature* layers, which is
probably the largest user-visible improvement available.
