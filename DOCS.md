...
| **Planning what comes after v1.0** | [docs/SWIFTKOTLIN_NATIVE_MIGRATION_GUIDE.md](docs/SWIFTKOTLIN_NATIVE_MIGRATION_GUIDE.md) or [docs/FLUTTER_MIGRATION_GUIDE.md](docs/FLUTTER_MIGRATION_GUIDE.md) — what a native rebuild would take, and the decisions it turns on |
...
## 3. `docs/` — forward-looking: native migration planning

Unlike the rest of this catalog, these two do not describe the code as it is —
they describe what carrying it forward as a native application would take. They
are written for a mobile architect sizing the work and a product owner deciding
what an MVP contains, and they are the right starting point for any conversation
about rebuilding rather than maintaining Engage MT.

| Doc | Purpose |
|---|---|
| [docs/SWIFTKOTLIN_NATIVE_MIGRATION_GUIDE.md](docs/SWIFTKOTLIN_NATIVE_MIGRATION_GUIDE.md) | The migration written as **two native applications** — iOS in Swift, Android in Kotlin. |
| [docs/FLUTTER_MIGRATION_GUIDE.md](docs/FLUTTER_MIGRATION_GUIDE.md) | The same migration written as **one Flutter codebase** compiled to both platforms. |

They share structure deliberately, so the sections can be read side by side, and
each contains the same three things: an inventory of what exists and what ports,
an honest comparison of what ships today against what a rebuild would deliver,
and a list of the decisions that belong to FWP rather than to any developer —
chief among them whether to publish **a single shared, pre-styled map service**,
which is the one change that makes every version of this app smaller.

## 4. `docs/rules/` — per-domain engineering conventions
...
## 5. `docs/regs-manager/` — the regulations backend handover
...
## 7. `docs/mobile/` — mobile build & release
...
## 8. `docs/stubs/` — FWP integration contracts
...
## 9. `docs/accessibility/` & `docs/security/` — compliance & posture
...
## 10. In-code `README.md` orientation notes
